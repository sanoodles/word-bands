...
| The tab walk steps back from `#main` before it starts | Blurring the autofocused field leaves Chrome's sequential-focus starting point on it, so a Tab from there lands on what follows the field and the six stops before it go unwalked. `#main` is where a click on empty page space lands, since it covers the whole page, and the skip link is the one focusable the body holds before it — so a single Shift+Tab reaches the top of the page whatever else moves |
...
| `TextInput.Root` | Writes `aria-invalid` from its own `status` **after** spreading our props, so ours is silently overridden and the field reads `false` beside its error | `status="error"`, which is the only way in. It brings a warning icon, left grey rather than red by the border override above |
...
| A picked or highlighted select row takes the row's own colour, not the muted one | No single grey holds 7:1 on all six fills. Dark would need `neutral-25`, which is the text colour itself and stops the code reading as muted at all; light would need `neutral-80`. So the two state rows are coloured per row and the plain ones stay muted |
| The skip link's two edges are drawn by different things | White text at 7:1 needs a fill too dark to also hold 3:1 against the dark page — **no fill does both jobs**. The fill draws the edge on the light page and the border draws it on the dark one, which is what `p6-skip.mjs`'s `borderVsPage` measures |
...
| The rows are inset by `RING` | The scroller clips whatever leaves it, so the first row and the first chip of every row lost the top and left of their focus ring (2.4.13). The rows sit 4px in and `packRows` is given 8px less width. `GAP` has to stay at least twice `RING`, or a row's ring no longer fits inside one `stride` — which is what the scroll and the virtualizer window both assume |
